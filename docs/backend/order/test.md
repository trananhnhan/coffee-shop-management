...
| serializer | CreateOrderSerializer | queue_number generation | 2nd takeaway order created same day, same branch | queue_number = previous count + 1 | |
...
- Takeaway `queue_number` generation counts today's existing takeaway orders and adds 1, with no unique constraint enforcing it — under concurrent order creation in the same branch/day, two requests could compute the same `count` and produce a duplicate queue_number (race condition). Not required to fix for a capstone deadline, but worth a comment in the test file noting it's a known limitation rather than silently assuming it's race-safe.